...
Markup should be fully encapsulated into the shadow element of the JavaScript object.
...
This is automatically generated by its `packagename` defined as its class variable by default, with appending sequencial number to be identical in global. 
If it is not defined, just used its class name.

The component instance is a JavaScript object. 
So, you can access to the instance via its `componentId` from anywhere in you want.
...
※ componentId has auto generated suffix serial number.
...
The Component is concretized by `Craft.UI.View` and `Craft.UI.ViewController`. 
Both are sub-class of `Craft.Core.Component`.
View is an element in your application packing its template, style and action. 
ViewController is also a kind of View having some View elements in it, and manages them.

Your application may also have special ViewController called RootViewController. 
...
Component has a DOM tree in `this.root` that is based on `template()`.
...
This is defined in `style()` method and it should return usual style sheet expression.
...
In `style` method, you may not use it, but sometimes it may be required to cascade styles of your super class. 
In `template` method, it is used for accessing its method.


## Component Method

Template, style and actions are encapsulated in the Component.
...
`DefaultRootViewController.didReceivePopstate` extracts hash fragment from the window.location, 
and pass it to `resolveRoutingRequest`. 
You have to implement your routing logic in it. Or you have to override `DefaultRootViewController.didReceivePopstate` as you like.
...
To start routing at booting, you have to call `didReceivePopstate` of your RootViewController. 
